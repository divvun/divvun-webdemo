...
var modesUrl = protocol + "//" + hostname + ":" + port.toString() + subdir + "/list?q=checkers";
var checkUrl = protocol + "//" + hostname + ":" + port.toString() + subdir + "/checker";
log(checkUrl);
var hunUrl = protocol + "//" + hostname + ":" + port.toString() + subdir + "/hunspell";
var errXmlUrl = protocol + "//" + hostname + window.location.pathname + "/errors/";
  ...
  return $.ajax(url, {
    ...
    error: function error(jqXHR, textStatus, errXHR) {
      ...
      if (textStatus === "abort" && jqXHR.status === 0) {
        ...
        l10n().formatValue('modemissing').then(function (t) {
          $("#serverfault").html(t).show();
        });
      } else {
...
var getModes = function getModes() {
  var _xhr = $.ajax(modesUrl, {
    ...
    data: {},
    success: function success(res) {
      var modelist = res.responseData;

      Array.from(groupBy(modelist, function (m) {
        return m["specLanguage"];
...
var modeToDropdown = function modeToDropdown(m) {
  var lang = m.specLanguage,
      variant = m.pipename;
  var a = $('<a>').text(lang + " " + variant).on('click', function (_ev) {
    window.location.search = '?lang=' + lang + "&variant=" + variant;
    updateVariantDropdown(lang, variant);
  });
  var li = $('<li class="mode ma2">').append(a).data("lang", lang).data("variant", variant);
  $('#modes').append(li);
};
...
var updateVariantDropdown = function updateVariantDropdown(lang, variant) {
...
var showIgnorables = function showIgnorables() {
  var search = searchToObject(),
      variant = getVariant(search),
      lang = getLang(search),
  ...
  $.ajax(url, {
    success: function success(res) {
      var ul = $("#ignorables-list");
      ul.html("");
      var errors = $(res.activeElement).find('error');
      errors.each(function (i, e) {
        var t = $(e).find('header title[xml\\:lang="se"]').text();
